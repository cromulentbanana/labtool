/**
 * jquery numberfiy textareas
 * http://www.mitya.co.uk/scripts/197/source/numberfy.js
 * with adjustments by Paul Grau
 */
(function($) { $.fn.numberfy = function() {
  ...
  $(this).filter('textarea').each(function() {
    ...
    var col = $('<div>').css({width: 10, position: 'absolute', overflow: 'hidden', left: 0, top: 0, height: $(this).height()}).addClass('lineNumsCol').prependTo($(this).parent());

    //line nums col needs to copy all line- and X/Y box-affecting styls of textarea
    var css = {}, thiss = $(this);
    $.each(['marginTop', 'marginBottom', 'paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft', 'borderTop', 'borderRight', 'borderBottom', 'borderLeft'], function(key, val) { css[val] = thiss.css(val); });
  ...
  function doLines(el) {
    ...
    var numbers = Math.ceil( Math.log(len + 1) / Math.log(10) );
    var width = numbers * 10;
    col.css('width', width);
    col.parent().css('padding-left', width + 14);
...
*
* Heavily modified by Paul Grau <paul@pgrau.de>, 2013
*/
(function ($) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        $('#range-selector .input-startDate').on('keyup', $.proxy(this.updateFromControl, this));
        $('#range-selector .input-endDate').on('keyup', $.proxy(this.updateFromControl, this));

        this.leftCalendar.month.set({ month: this.startDate.getMonth(), year: this.startDate.getFullYear() });
        this.rightCalendar.month.set({ month: this.startDate.getMonth() + 1, year: this.startDate.getFullYear() });
    ...
    DateRangePicker.prototype = {
        ...
        updateView: function () {
            var startString = this.startDate.toString(this.format);
            if (this.startDate - Date.now() < 60) {
              // Reset "nearly now" as well as past dates to now.
              startString = "now";
              this.startDate = new Date( Date.now() );
            }
            $('#range-selector .input-startDate').val(startString);
            $('#range-selector .input-endDate').val(this.endDate.toString(this.format));

        },

        updateFromControl: function () {
            if ($('#range-selector .input-startDate').val() == "now") {
              var start = new Date( Date.now() );
            } else {
              var start = Date.parseExact($('#range-selector .input-startDate').val(), this.format);
            }
            var end = Date.parseExact($('#range-selector .input-endDate').val(), this.format);
        ...
        clickDate: function (e) {
            ...
            if (cal.is('.left'))
              selected_date = this.leftCalendar.calendar[row][col];
            else
              selected_date = this.rightCalendar.calendar[row][col];

            if (this.nextClick=='start')
                startDate = selected_date.set({  hour: startDate.getHours(), minute: startDate.getMinutes() });
            else
                endDate = selected_date.set({  hour: endDate.getHours(), minute: endDate.getMinutes() });
            ...
            //this.leftCalendar.month.set({ month: this.startDate.getMonth(), year: this.startDate.getFullYear() });
            //this.rightCalendar.month.set({ month: this.startDate.getMonth() + 1, year: this.endDate.getFullYear() });
            this.updateView();
            this.updateCalendars();

            this.cb(this.startDate, this.endDate);

            this.nextClick = (this.nextClick == 'start' ? 'end' : "start");
            console.log("Next click is gonna be " + this.nextClick);
        },
        ...
        updateCalendars: function () {
            ...
            this.rightCalendar.calendar = this.buildCalendar(this.leftCalendar.month.getMonth() + 1, this.rightCalendar.month.getFullYear());
        ...
        renderCalendar: function (calendar, selected, minDate, maxDate,lor) {
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    var startDate = this.startDate.clone().setHours(0,0,0,0),
                        endDate = this.endDate.clone().setHours(0,0,0,0);

                    var this_day = calendar[row][col].setHours(0,0,0,0)
                    /*if ( (minDate && calendar[row][col] < minDate) || (maxDate && calendar[row][col] > maxDate))
                    {
                        cname = 'off disabled';
                        
                    }
                    else */if (this_day == startDate || this_day == endDate)
                    {
                        cname += ' active';
                    }
                    if (this_day >= startDate && this_day <= endDate) {
                        cname += ' in-range';
                    }
